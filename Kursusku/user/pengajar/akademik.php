...
$no=1;
$queriA="Select * From pengajar where email_pengajar='$username'" ;
$hasilA=MySQLi_query ($konek_db, $queriA);
$row=mysqli_fetch_array ($hasilA);
$mapel=$row['nama_mapel'];

$queriB="Select * From matpel where nama_mapel='$mapel'" ;
...
while ($data = mysqli_fetch_array ($hasilB)){
 ...
 <tbody>
      <tr>
        ...
        <td><a href='list/nama_siswa.php?kode_mapel=".$data['kode_mapel']."'>".$data['nama_mapel']."</a></td>
				<td>".$data['hari']."</td>
				<td>".$data['jam']."</td>
        <td>".$data['paket']."</td>
      </tr>
